...
object ZCREDMethods {
    fun enterZCRED(input: InputData, zc2redScope: ZC2REDScope): List<CreationData> {
        val options: List<ReducerOptionTemplate> = allReducersOptions.tryToCalculateOptions(inputData = input)
        var creationDataList: MutableList<CreationData> = arrayListOf()
        if (input.IST == 1)
            return ZC1RED(input = input,
                zc2redScope = zc2redScope,
                options = options,
                creationDataList = creationDataList)
        else
            return ZC2RED(input = input,
                zc2redScope = zc2redScope,
                options = options,
                creationDataList = creationDataList
            )
    ...
    private fun ZC1RED(
        ...
        options.forEach {option ->
            option.apply {
                ...
                zc2redScope.TVL2 = 9550f*option.EDScope!!.PED!!*input.U0*u*input.KPD/option.EDScope.NED!!
                ...
                val creationData = CreationData(edScope = option.EDScope,
                    option = option,
                    gearWheelStepsArray = arrayOf(onlyStep))//Возвращаем только 1 ступень
                creationDataList.add(creationData)
            }
        }
        return creationDataList
