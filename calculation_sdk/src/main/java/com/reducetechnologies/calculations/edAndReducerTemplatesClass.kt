...
class AllReducersOptionsClass(val tableHRC: HRCTable, val tableEDData: EDDataTable) {
    /**
    ...
    private fun enterOptions(inputData: InputData, pedCalculated: Float): List<ReducerOptionTemplate> {
        ...
        for (ned in NEDOptions) {
            ...
            if (inputData.isED) {
                EDMethodsClass(tableEDData).EDCalculate(
            ...
            } else inputData.UREMA //если не подбираем электродвигатель или его невозможно подобрать
