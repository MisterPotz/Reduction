...
internal class CalculationsModuleTest {
    ...
    fun weedOutCreationDataList(
        inputData: InputData,
        creationDataList: List<CreationData>,
        masreScopeList: ArrayList<MasreScope>
    ): ArrayList<CreationData> {
    ...
    fun weedOutMasreDataList(
        inputData: InputData,
        creationDataList: List<CreationData>,
        masreScopeList: ArrayList<MasreScope>
    ): ArrayList<MasreScope> {
        val properCreationDataArrList: ArrayList<CreationData> = arrayListOf()
        val newMasreScopeList: ArrayList<MasreScope> = arrayListOf()
        creationDataList.forEachIndexed loop@{ i, creationData ->
            creationData.gearWheelStepsArray.forEachIndexed { j, oneGearWheelStep ->
    ...
    fun setUp() {
        ...
        val databaseModule = DatabaseModule(context, locale)
        ...
        val calculationSetup = DaggerCalculationSetupComponent.builder()
            .calculationSetup(CalculationSetup())
            .databaseModule(databaseModule).build()
        runBlocking {
            calculationSetup.getStorage().init()
        }
        calculationComponent =
            DaggerCalculationsComponent.builder()
                .calculationModule(CalculationModule(calculationSetup.getStorage().obtain()))
                .build()
    ...
    fun test_dopn_methods() {
        val reducersOptions = allReducersMethod.tryToCalculateOptions(inputData = input)
        val dopnArguments = DOPN_MethodsClass.Arguments(
            N2 = 190f,
            u = 7.04f,
            inputData = input,
